...
const jsonRuta = esPaginaSecundaria ? "../js/textos.json" : "js/textos.json";
...
function buscarPalabra(palabra, paginaActual, esRedireccion = false) {
  ...
  // Normalizar nombre de la página
  ...
  if (textos[paginaActual]) {
    ...
    bloques.forEach(b => {
      ...
      if (original.toLowerCase().includes(palabra)) {
        b.innerHTML = original.replace(regex, `<span class="resaltado">$1</span>`);
        encontrada = true;
  ...
  if (!encontrada && !esRedireccion) {
    for (const [nombreArchivo, textosPagina] of Object.entries(textos)) {
      ...
      const coincide = textosPagina.some(t => t.toLowerCase().includes(palabra));

      if (coincide) {
        let rutaFinal = esPaginaSecundaria 
          ? (nombreArchivo === "index.html" ? "../index.html" : nombreArchivo)
          : (nombreArchivo === "index.html" ? "index.html" : "pages/" + nombreArchivo);

        window.location.href = `${rutaFinal}?buscar=${encodeURIComponent(palabra)}`;
        return;
      }
    }
